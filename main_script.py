...
class DataProcessingToolGUI:
    ...
    def create_button_frame(self):
        ...
        self.load_csv_btn = ttk.Button(
            self.button_frame, text="Load CSV", command=self.load_csv
        )
    ...
    def create_filter_frame(self):
        ...
        filter_check = ttk.Checkbutton(
            ...
            variable=filter_var,
            state="disabled",
            command=lambda: self.toggle_row_display(
                3
            ),  # Assuming this is the fourth filter
        )
    ...
    def create_table_frame(self):
        ...
        )  # Changed row to 0 and added rowspan
        self.table_frame.columnconfigure(0, weight=1)
        self.table_frame.rowconfigure(0, weight=1)
        self.table = Table(
            self.table_frame, dataframe=self.df, showtoolbar=True, showstatusbar=True
        )
        self.table.show()

    def create_log_window(self):
        self.log_window = tk.Text(
            self.main_frame, height=4, state="disabled", bg="lightgray"
        )
    ...
    def toggle_row_display(self, toggle_index=None):
        try:
            ...
            active_filter_index = next(
                (i for i, var in enumerate(self.filter_vars) if var.get()), None
            )
            ...
            filtered_df = self.controller.get_filtered_data_based_on_toggle(
                active_filter_index
            )
